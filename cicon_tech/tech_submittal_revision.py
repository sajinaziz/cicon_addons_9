...
class SubmittalRevision(models.Model):
    ...
    def total_delivery(self):
        ...
        """
        self.delivered_qty = sum([d.delivered_qty for d in self.delivery_ids])
        self.balance_qty = self.bbs_weight - self.delivered_qty
        self.dwg_count = len(self.document_ids)
        self.total_draft_time = sum(x.draft_time for x in self.document_ids)
    ...
    ref_no = fields.Char('Ref No', size=50, required=True, readonly=True, states={'new': [('readonly', False)]})
